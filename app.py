import streamlit as st

# --- Διαμόρφωση Σελίδας ---
st.set_page_config(page_title="Gym-Bot: Ανέβα Επίπεδο!", page_icon="🏆")

# --- SIDEBAR: Λογότυπο & Πληροφορίες ---
with st.sidebar:
    logo_url = "https://raw.githubusercontent.com/GiorgosBouh/test_sub8/main/logo1.png"
    st.image(logo_url, width=150)
    st.divider()
    st.caption(
        "Αυτή η εφαρμογή αναπτύχθηκε τον Νοέμβριο του 2025 στα πλαίσια της επιμόρφωσης "
        "από τον **Γεώργιο Μπουχουρά**."
    )
    st.caption(
        "Μπορεί να χρησιμοποιηθεί ελεύθερα για τους σκοπούς που αναφέρονται "
        "από όλους τους εκπαιδευτικούς."
    )
    st.caption("Άδεια Χρήσης: [Creative Commons CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)")

# --- ΑΡΧΙΚΟΠΟΙΗΣΗ "ΜΝΗΜΗΣ" (Session State) ---
if 'analysis_done' not in st.session_state:
    st.session_state.analysis_done = False
if 'plan_text' not in st.session_state:
    st.session_state.plan_text = ""
if 'goal' not in st.session_state:
    st.session_state.goal = ""
if 'analysis_output' not in st.session_state:
    st.session_state.analysis_output = []
if 'scores' not in st.session_state:
    st.session_state.scores = {}


# --- ΚΥΡΙΑ ΕΦΑΡΜΟΓΗ ---
st.title("🏆 Gym-Bot: Ανέβα Επίπεδο!")
st.write(f"Γεια, μελλοντικέ ήρωα! Είμαι ο Gym-Bot 🤖")
st.write("Έτοιμος να δούμε τα **Χαρακτηριστικά** σου; Φέρε το **'Φύλλο Αποστολής'** από την αυλή και πάμε!")

# --- 1. ΕΙΣΑΓΩΓΗ ΔΕΔΟΜΕΝΩΝ (C) ---
st.header("📋 Αποστολή 1: Καταχώρησε Χαρακτηριστικά")

col1, col2, col3 = st.columns(3)
with col1:
    pushups = st.number_input("Κάμψεις (ΔΥΝ);", min_value=0, step=1)
    feeling_pushups = st.selectbox(
        "Βαθμός Δυσκολίας (Κάμψεις);",
        ["", "Πανεύκολο", "Ισορροπημένο", "Δύσκολο", "Επίπεδο 'Αφεντικού'!"], key="feel_pushups"
    )
with col2:
    plank = st.number_input("Σανίδα (ΠΥΡ) (δευτ.);", min_value=0, step=1)
    feeling_plank = st.selectbox(
        "Βαθμός Δυσκολίας (Σανίδα);",
        ["", "Πανεύκολο", "Ισορροπημένο", "Δύσκολο", "Επίπεδο 'Αφεντικού'!"], key="feel_plank"
    )
with col3:
    sprints = st.number_input("Γόνατα ψηλά (ΑΝΤ);", min_value=0, step=1)
    feeling_sprints = st.selectbox(
        "Βαθμός Δυσκολίας (Γόνατα);",
        ["", "Πανεύκολο", "Ισορροπημένο", "Δύσκολο", "Επίπεδο 'Αφεντικού'!"], key="feel_sprints"
    )


# --- 2. ΚΟΥΜΠΙ ΑΝΑΛΥΣΗΣ ---
if st.button("🚀 Ανάλυση Χαρακτηριστικών!"):
    if not feeling_pushups or not feeling_plank or not feeling_sprints or "" in [feeling_pushups, feeling_plank, feeling_sprints]:
        st.warning("Ουπς! Πρέπει να συμπληρώσεις *όλα* τα χαρακτηριστικά σου για να συνεχίσεις!")
        st.session_state.analysis_done = False
    else:
        st.session_state.analysis_done = True
        
        # Υπολογισμός "Score"
        pushup_score = min(int((pushups / 20.0) * 100), 100) 
        plank_score = min(int((plank / 60.0) * 100), 100)
        sprint_score = min(int((sprints / 50.0) * 100), 100) # π.χ. 50 επαναλ. = 100%
        st.session_state.scores = {'ΔΥΝ': pushup_score, 'ΠΥΡ': plank_score, 'ΑΝΤ': sprint_score}
        
        analysis_texts = []
        if "Δύσκολο" in feeling_pushups or "Αφεντικού" in feeling_pushups or pushups < 10:
            analysis_texts.append(("info", "💪 **ΔΥΝ (Δύναμη):** Καλή προσπάθεια! Οι κάμψεις ήταν πρόκληση. Αυτή είναι η ικανότητα που θα δυναμώσεις! +10 Πόντοι Εμπειρίας (ΠΕ) για την προσπάθεια!"))
        else:
            analysis_texts.append(("success", "💪 **ΔΥΝ (Δύναμη):** Ωραίος! Έχεις ήδη καλή ικανότητα στη Δύναμη. Έτοιμος για το επόμενο επίπεδο!"))

        if "Δύσκολο" in feeling_plank or "Αφεντικού" in feeling_plank or plank < 20:
            analysis_texts.append(("info", "🧘 **ΠΥΡ (Πυρήνας):** Η σανίδα ήταν 'Επίπεδο Αφεντικού'! Ο πυρήνας (κοιλιά/ράχη) είναι η βάση σου. Χρειάζεται προπόνηση!"))
        else:
            analysis_texts.append(("success", "🧘 **ΠΥΡ (Πυρήνας):** Ατσάλινος! Ο πυρήνας σου είναι πολύ δυνατός!"))

        if "Δύσκολο" in feeling_sprints or "Αφεντικού" in feeling_sprints or sprints < 40:
            analysis_texts.append(("info", "⚡ **ΑΝΤ (Αντοχή):** Λαχάνιασες; Η αντοχή είναι κλειδί για το παιχνίδι. Καλός στόχος για εξάσκηση!"))
        else:
            analysis_texts.append(("success", "⚡ **ΑΝΤ (Αντοχή):** Τρέχεις σαν τον άνεμο! Εξαιρετική αντοχή."))
        
        st.session_state.analysis_output = analysis_texts
        st.session_state.goal = ""
        st.session_state.plan_text = ""

# --- 3. ΕΜΦΑΝΙΣΗ ΑΝΑΛΥΣΗΣ ΚΑΙ ΣΤΟΧΩΝ ---
if st.session_state.analysis_done:
    st.header("📊 Η Αναφορά σου")
    
    st.write("Ικανότητα Δύναμης (ΔΥΝ):")
    st.progress(st.session_state.scores.get('ΔΥΝ', 0))
    st.write("Ικανότητα Πυρήνα (ΠΥΡ):")
    st.progress(st.session_state.scores.get('ΠΥΡ', 0))
    st.write("Ικανότητα Αντοχής (ΑΝΤ):")
    st.progress(st.session_state.scores.get('ΑΝΤ', 0))
    
    for msg_type, text in st.session_state.analysis_output:
        if msg_type == "info":
            st.info(text)
        else:
            st.success(text)

    st.header("🎯 Αποστολή 2: Βάλε Στόχο")
    st.write("Τώρα που είδες τα χαρακτηριστικά σου, γράψε τη νέα σου αποστολή!")
    
    goal_input = st.text_area("Γράψε την Αποστολή σου (π.χ. 'Να ανέβω επίπεδο στις κάμψεις, να κάνω 3 παραπάνω')", key="goal_text_area")

    if st.button("📜 Κλείδωμα Αποστολής!"):
        if not goal_input:
            st.warning("Πρέπει να γράψεις την Αποστολή σου πρώτα!")
            st.session_state.plan_text = ""
        else:
            st.session_state.goal = goal_input
            st.balloons()
            st.success(f"Νέα Αποστολή: '{st.session_state.goal}'! Λαμβάνεις 'Πάπυρο Προπόνησης'!")
            
            plan_text = f"## 📜 Πάπυρος Προπόνησης 📜\n\n"
            plan_text += f"**Αποστολή:** {st.session_state.goal}\n\n"
            
            # Δημιουργία πλάνου
            if "κάμψεις" in st.session_state.goal.lower():
                plan_text += "Για να πετύχεις αυτή την Αποστολή, προτείνω αυτό το **Πλάνο Προπόνησης**:\n"
                plan_text += "1. 🗓️ **Συχνότητα:** 3 φορές την εβδομάδα (π.χ. Δευτέρα-Τετάρτη-Παρασκευή).\n"
                plan_text += "2. 🏋️ **Σετ:** 3 σετ κάμψεις (μπορείς να βάζεις τα γόνατα κάτω αν κουράζεσαι).\n"
                plan_text += "3. 📈 **Επαναλήψεις:** Σε κάθε σετ, κάνε όσες μπορείς! Προσπάθησε την επόμενη φορά να κάνεις +1!"
                
            elif "σανίδα" in st.session_state.goal.lower():
                plan_text += "Για να πετύχεις αυτή την Αποστολή, προτείνω αυτό το **Πλάνο Προπόνησης**:\n"
                plan_text += "1. 🗓️ **Συχνότητα:** 4 φορές την εβδομάδα (ακόμα και για 1 λεπτό την ημέρα).\n"
                plan_text += "2. 🏋️ **Σετ:** 3 σετ σανίδα.\n"
                plan_text += "3. 📈 **Χρόνος:** Προσπάθησε να κρατήσεις 5 δευτερόλεπτα παραπάνω σε κάθε σετ!"
            
            elif "αντοχή" in st.session_state.goal.lower() or "τρέξιμο" in st.session_state.goal.lower() or "γόνατα" in st.session_state.goal.lower():
                plan_text += "Για να πετύχεις αυτή την Αποστολή (ΑΝΤ), προτείνω αυτό το **Πλάνο Προπόνησης**:\n"
                plan_text += "1. 🗓️ **Συχνότητα:** 3-4 φορές την εβδομάδα.\n"
                plan_text += "2. 🏋️ **Σετ:** 4 σετ 'Επιτόπιο τρέξιμο' για 1 λεπτό το καθένα.\n"
                plan_text += "3. 📈 **Μπόνους:** Παίξε κυνηγητό ή μπάσκετ στο διάλειμμα. Αυτή είναι η καλύτερη εξάσκηση για Αντοχή!"

            else:
                plan_text += "Αυτή είναι Επική Αποστολή! Για να την πετύχεις, θυμήσου τον χρυσό κανόνα:\n"
                plan_text += "1. 🗓️ **Συνέπεια:** Κάνε κάτι κάθε μέρα (π.χ. 10 λεπτά άσκηση).\n"
                plan_text += "2. 🔥 **Ένταση:** Πρέπει να λαχανιάζεις λιγάκι (να νιώθεις την προσπάθεια!).\n"
                plan_text += "3. 🍎 **Καλή διατροφή:** Μην ξεχνάς τα 'Φίλτρα Ζωής' (φρούτα/νερό)!"
            
            st.session_state.plan_text = plan_text
            
    # Εμφάνιση του πλάνου (αν υπάρχει στη μνήμη)
    if st.session_state.plan_text:
        st.markdown(st.session_state.plan_text, unsafe_allow_html=True)
        st.download_button(
            label="📥 Κατέβασε την Αποστολή σου!",
            data=st.session_state.plan_text,
            file_name=f"Η_Αποστολή_Μου.txt"
        )
        
        st.divider()
        st.header("✨ ΜΠΟΝΟΥΣ: Το 'Μυστικό' σου")
        st.write("Τώρα που έχεις την Αποστολή σου, πήγαινε στο Google Doc / Padlet που σου έδωσε ο δάσκαλος και γράψε τη δική σου 'Χρυσή Συμβουλή' από την ώρα της αυλής!")