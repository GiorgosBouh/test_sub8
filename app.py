...
st.write(f"Γεια, μελλοντικέ αθλητή! Είμαι ο Gym-Bot 🤖")
st.write("Έτοιμος να δούμε τα 'Stats' σου; Φέρε το **'Φύλλο Αποστολής'** από την αυλή και πάμε!")

# --- 1. ΕΙΣΑΓΩΓΗ ΔΕΔΟΜΕΝΩΝ (C) ---
st.header("📋 Αποστολή 1: Ανέβασε Stats")

col1, col2, col3 = st.columns(3)
with col1:
    pushups = st.number_input("Κάμψεις (ΔΥΝ);", min_value=0, step=1)
    feeling_pushups = st.selectbox(
        "Βαθμός Δυσκολίας (Κάμψεις);",
        ["", "Πανεύκολο", "Ισορροπημένο", "Δύσκολο", "Επίπεδο 'Boss'!"], key="feel_pushups"
    )
with col2:
    plank = st.number_input("Σανίδα (ΠΥΡ) (δευτ.);", min_value=0, step=1)
    feeling_plank = st.selectbox(
        "Βαθμός Δυσκολίας (Σανίδα);",
        ["", "Πανεύκολο", "Ισορροπημένο", "Δύσκολο", "Επίπεδο 'Boss'!"], key="feel_plank"
    )
with col3:
    sprints = st.number_input("Γόνατα ψηλά (ΑΝΤ);", min_value=0, step=1)
    feeling_sprints = st.selectbox(
        "Βαθμός Δυσκολίας (Γόνατα);",
        ["", "Πανεύκολο", "Ισορροπημένο", "Δύσκολο", "Επίπεδο 'Boss'!"], key="feel_sprints"
    )


# --- 2. ΚΟΥΜΠΙ ΑΝΑΛΥΣΗΣ ---
if st.button("🚀 Ανάλυση Stats!"):
    if not feeling_pushups or not feeling_plank or not feeling_sprints or "" in [feeling_pushups, feeling_plank, feeling_sprints]:
        ...
        analysis_texts = []
        if "Δύσκολο" in feeling_pushups or "Boss" in feeling_pushups or pushups < 10:
            analysis_texts.append(("info", "💪 **ΔΥΝ (Δύναμη):** Καλή προσπάθεια! Οι κάμψεις ήταν πρόκληση. Αυτό είναι το skill που θα 'farm-άρεις' (βελτιώσεις)! +10 XP για την προσπάθεια!"))
        else:
            analysis_texts.append(("success", "💪 **ΔΥΝ (Δύναμη):** Ωραίος! Έχεις ήδη καλό skill στη Δύναμη. Έτοιμος για το επόμενο level!"))

        if "Δύσκολο" in feeling_plank or "Boss" in feeling_plank or plank < 20:
            analysis_texts.append(("info", "🧘 **ΠΥΡ (Πυρήνας):** Η σανίδα ήταν 'Επίπεδο Boss'! Ο πυρήνας (κοιλιά/ράχη) είναι η βάση σου. Χρειάζεται training!"))
        else:
            analysis_texts.append(("success", "🧘 **ΠΥΡ (Πυρήνας):** Ατσάλι! Ο πυρήνας σου είναι 'tank'! Πολύ καλό stat!"))

        if "Δύσκολο" in feeling_sprints or "Boss" in feeling_sprints or sprints < 40:
            analysis_texts.append(("info", "⚡ **ΑΝΤ (Αντοχή):** Λαχάνιασες; Η αντοχή (Stamina) είναι κλειδί για το παιχνίδι. Καλός στόχος για 'grinding' (εξάσκηση)!"))
        else:
...
if st.session_state.analysis_done:
    ...
    st.write("Τώρα που είδες τα stats, γράψε τη νέα σου αποστολή!")
    
    goal_input = st.text_area("Γράψε την Αποστολή σου (π.χ. 'Level up στις κάμψεις, να κάνω 3 παραπάνω')", key="goal_text_area")
    ...
    if st.button("📜 Κλείδωμα Αποστολής!"):
        if not goal_input:
            ...
            if "κάμψεις" in st.session_state.goal.lower() or "pushups" in st.session_state.goal.lower():
                plan_text += "Για να πετύχεις αυτή την Αποστολή, προτείνω αυτό το **Πλάνο Προπόνησης**:\n"
                plan_text += "1. 🗓️ **Συχνότητα:** 3 φορές την εβδομάδα (π.χ. Δευτέρα-Τετάρτη-Παρασκευή).\n"
                plan_text += "2. 🏋️ **Σετ:** 3 σετ κάμψεις (μπορείς να βάζεις τα γόνατα κάτω αν κουράζεσαι).\n"
                plan_text += "3. 📈 **Επαναλήψεις:** Σε κάθε σετ, κάνε όσες μπορείς! Προσπάθησε την επόμενη φορά να κάνεις +1!"
                ...
                plan_text += "2. 🏋️ **Σετ:** 3 σετ σανίδα.\n"
                plan_text += "3. 📈 **Χρόνος:** Προσπάθησε να κρατήσεις 5 δευτερόλεπτα παραπάνω σε κάθε σετ!"
                ...
                plan_text += "Για να πετύχεις αυτή την Αποστολή (ΑΝΤ), προτείνω αυτό το **Πλάνο Προπόνησης**:\n"
                plan_text += "1. 🗓️ **Συχνότητα:** 3-4 φορές την εβδομάδα.\n"
                plan_text += "2. 🏋️ **Σετ:** 4 σετ 'Επιτόπιο τρέξιμο' για 1 λεπτό το καθένα.\n"
                plan_text += "3. 📈 **Bonus:** Παίξε κυνηγητό ή μπάσκετ στο διάλειμμα. Αυτό είναι το καλύτερο 'grinding' για Αντοχή!"
    ...
    if st.session_state.plan_text:
        ...
        st.divider()
        st.header("✨ BONUS: Το 'How-To' σου")
        st.write("Τώρα που έχεις την Αποστολή σου, πήγαινε στο Google Doc / Padlet που σου έδωσε ο δάσκαλος και γράψε το δικό σου 'Pro-Tip' από την ώρα της αυλής!")